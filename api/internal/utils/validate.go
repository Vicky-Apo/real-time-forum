...
func ValidateNickname(nickname string) error {
	// Nickname validation using configuration (same rules as username)
	if len(nickname) < config.Config.MinUsernameLen || len(nickname) > config.Config.MaxUsernameLen {
		return errors.New("nickname must be between " +
			string(rune(config.Config.MinUsernameLen)) + " and " +
			string(rune(config.Config.MaxUsernameLen)) + " characters")
	}

	// Nickname character validation (alphanumeric and underscore only)
	nicknameRegex := regexp.MustCompile("^[a-zA-Z0-9_]+$")
	if !nicknameRegex.MatchString(nickname) {
		return errors.New("nickname can only contain alphanumeric characters and underscores")
	}
...
func ValidateUserInput(nickname, email, password, gender, firstName, lastName string, age int) error {
